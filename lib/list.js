...
function validate(opts) {
  if (opts.category && typeof opts.category !== 'number') {
    throw Error('Category must be a number');
  }
...
function list(opts) {
  return new Promise(function (resolve, reject) {
    ...
    const categoryPath = opts.category ? `/genre=${opts.category}` : '';
    const url = `https://itunes.apple.com/${opts.country}/rss/${opts.collection}${categoryPath}/limit=${opts.num}/json`;
