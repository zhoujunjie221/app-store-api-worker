...
const doRequestViaProxy = async (url, headers, requestOptions, limit) => {
  console.log('Trying proxy request method for:', url);
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.contents) {
        console.log('Proxy request successful via allorigins');
...
const doRequestDirect = async (url, headers, requestOptions, limit) => {
  console.log('Trying direct request with different strategy for:', url);
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('Direct alternative request successful');
      return body;
    }

    throw new Error(`Direct alternative request failed: ${response.status}`);
  } catch (error) {
    console.log('Direct alternative request failed:', error.message);
    throw error;
  }
};
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        console.log('Primary request blocked, trying alternative methods...');

        // Try direct alternative first
        try {
          console.log('Trying direct alternative method...');
          return await doRequestDirect(url, headers, requestOptions, limit);
        } catch (directError) {
          console.log('Direct alternative failed:', directError.message);
        }
        ...
        console.log('All alternative methods failed, falling back to original error');
      }
